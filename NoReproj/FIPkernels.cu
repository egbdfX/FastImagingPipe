...
__global__ void finalinterp(float* output_index, float* dirty_pre, float* dirty, size_t image_size) {
    ...
    if (idx < image_size && idy < image_size) {
		...
		if (abs(LL) < half_image_size-1 && abs(MM)<half_image_size-1) {
            ...
            atomicAdd(&dirty[static_cast<long int>(image_index_offset_image_centre+ceil_device(MM)*image_size+floor_device(LL))],(1-LL+floor_device(LL))*(MM-floor_device(MM))*dirty_pre[static_cast<long int>(image_index_offset_image_centre+idy*image_size+idx)]);
            atomicAdd(&dirty[static_cast<long int>(image_index_offset_image_centre+floor_device(MM)*image_size+ceil_device(LL))],(LL-floor_device(LL))*(1-MM+floor_device(MM))*dirty_pre[static_cast<long int>(image_index_offset_image_centre+idy*image_size+idx)]);
            atomicAdd(&dirty[static_cast<long int>(image_index_offset_image_centre+ceil_device(MM)*image_size+ceil_device(LL))],(LL-floor_device(LL))*(MM-floor_device(MM))*dirty_pre[static_cast<long int>(image_index_offset_image_centre+idy*image_size+idx)]);
		}
	}
}
...
int FIpipe(float* Visreal, float* Visimag, float* Bin, float* Vin, float* dirty_image, size_t num_baselines, size_t image_size, float freq_hz, float uv_scale, float phase_ra, float phase_dec){
	...
	float cell_size = uv_scale/grid_size;
